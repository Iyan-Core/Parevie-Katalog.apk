...
const SL = {
  ...
  paid_pending_confirm:"💰 Pembayaran diverifikasi",
...
function OrderModal({p}) {
  ...
  const submitOrder = async () => {
    ...
    try {
      const oRef = await addDoc(collection(db,"orders"),{
        ...
        buyerConfirmed: false, createdAt:serverTimestamp(),
      });
      setOId(oRef.id);
  ...
  useEffect(()=>{
    ...
    const u2=onSnapshot(doc(db,"orders",orderId),snap=>{
  ...
  const handleBuyerConfirm = async () => {
    ...
    try {
      ...
      const arr = lsGet().map(o=>o.orderId===orderId ? {...o, status:"done", buyerConfirmed:true} : o);
      lsSet(arr);
    } catch(e){ alert("Gagal konfirmasi: "+e.message); }
    setConfirming(false);
  };
  ...
  const curStatus = ordData?.status || "pending";
  const curConfirmed = ordData?.buyerConfirmed || false;

  return (
    <div className="chat-wrap">
      <div className="chat-hdr"><Ic.Chat/> <span>Chat dengan Admin</span>
        <span className="chat-status">● Online</span></div>
      <div className="status-bar" style={{background:SC[curStatus]+"22",borderColor:SC[curStatus]+"55",color:SC[curStatus]}}>
        {SL[curStatus]||curStatus}
      </div>

      {curStatus === "shipped" && !curConfirmed && (
        <div className="buyer-confirm-box">
          <button className="btn-buyer-confirm" onClick={handleBuyerConfirm} disabled={confirming}>
            {confirming?"⏳ Mengonfirmasi…":"✅ Saya Sudah Menerima Pesanan"}
          </button>
        </div>
      )}

      {curStatus === "done" && curConfirmed && (
        <div className="buyer-thankyou-box">
          ...
          <a href={`https://wa.me/${WA_ADMIN}?text=Halo+Parevie%2C+saya+butuh+bantuan+mengenai+pesanan+saya`}
            target="_blank" rel="noopener noreferrer" className="btn-wa-help">
